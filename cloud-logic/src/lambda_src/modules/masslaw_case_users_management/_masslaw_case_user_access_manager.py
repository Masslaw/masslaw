from src.lambda_src.modules.masslaw_cases_config._access_config import *
from src.lambda_src.modules.masslaw_cases_objects.masslaw_case import MasslawCaseInstance
from src.lambda_src.modules.masslaw_users_objects._masslaw_user_instance import MasslawUserInstance
from ...util import json_utils


class MasslawCaseUnauthorizedUserActionException(Exception): pass
...
class MasslawCaseUserAccessManager:
    ...
    def get_formatted_case_instance_for_entity(self, access_level):
        ...
        entity_permitted_data_keys = self.get_permitted_keys_for_access(AccessActions.READ, access_level)
    ...
    def update_data_as_entity(self, access_level, update_obj):
        entity_permitted_data_keys = self.get_permitted_keys_for_access(AccessActions.EDIT, access_level)
        update_obj = json_utils.select_keys(update_obj, entity_permitted_data_keys)
    ...
    def set_case_user_permissions_as_user(self, case_user_id, case_user_permission_level, case_user_restrictions):
        ...
        self.set_case_user_permissions_as_entity(user_access_level, case_user_id, case_user_permission_level,
                                                 case_user_restrictions)

    def set_case_user_permissions_as_entity(self, as_entity, case_user_id, case_user_permission_level,
                                            case_user_restrictions):
        ...
        if case_user_id in case_users:
            ...
            if level == CaseAccessEntities.OWNER_CLIENT:
                # if the user that's requested to change is the owner of the case, return and block the change
                return False
            if level == CaseAccessEntities.MANAGER_CLIENT and as_entity not in [CaseAccessEntities.OWNER_CLIENT]:
    ...
    def set_case_user_permissions(self, case_user_id, case_user_permission_level, case_user_restrictions):
        ...
        self.__case_instance.set_data_property(['users', case_user_id], {
            'access_level': case_user_permission_level,
            'access_restrictions': case_user_restrictions,
        })

        user_instance = MasslawUserInstance(case_user_id)
        user_instance.set_data_property(['cases', self.__case_instance.get_case_id()], {
            'access_level': case_user_permission_level
        })
    ...
    def get_user_access_files(self, user_id):
        ...
        if access_level in [CaseAccessEntities.OWNER_CLIENT,
                            CaseAccessEntities.MANAGER_CLIENT]:
            all_files = self.__case_instance.get_data_property(['files'], [])
            return all_files
        if access_level in [CaseAccessEntities.EXTERNAL_CLIENT]:
            return []
    ...
    def determine_can_upload_file(self, user_id):
        ...
        if user_access_level in [CaseAccessEntities.OWNER_CLIENT, CaseAccessEntities.MANAGER_CLIENT]:
            return True
        elif user_access_level in [CaseAccessEntities.EDITOR_CLIENT]:
    ...
    def get_permitted_keys_for_access(self, for_action, as_entity):
        return json_utils.get_from(ACCESS_PERMITTED_KEYS, [for_action, as_entity])
    ...
    def determine_user_access_level(self, user_id):
        return self.__case_instance.get_data_property(['users', user_id, 'access_level'],
