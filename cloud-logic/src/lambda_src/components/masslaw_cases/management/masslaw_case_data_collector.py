...
from ...util.dynamodb_data_holder import *
from ...masslaw_cases.management.masslaw_case_data_formatting import *
...
class MasslawCaseDataCollector:
    ...
    def get_case_data(self):
        case_data = get_case_data_full_format_from_db_item(self.__case_instance.get_data_property([], {}), self.__access_user_id or '')
        return case_data

    def get_case_base_data(self):
        case_data = get_case_data_base_format_from_db_item(self.__case_instance.get_data_property([], {}), self.__access_user_id or '')
        return case_data
    ...
    def get_case_files_data(self):
        ...
        return [get_case_file_data_base_format_from_db_item(item_data=item_data) for item_data in items_data]

    def get_file_data(self, file_id):
        case_files = self.__case_instance.get_data_property(['files'], [])
        if not file_id in case_files:
            raise MasslawCaseUnauthorizedUserActionException(f"An attempt was made to get the data of a file which is not included in a user's case permissions")

        table_manager = DynamoDBTableManager("MasslawFiles")
        item_data = table_manager.get_item(file_id)
        file_data = get_case_file_data_full_format_from_db_item(item_data)
        return file_data
