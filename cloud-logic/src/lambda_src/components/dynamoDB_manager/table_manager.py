...
class DynamoDBTableManager:
    ...
    def get_item(self, key, default=None):
        ...
        item = json_utils.ensure_dict(item)
        return item
    ...
    def delete_item(self, key):
        key = self.__ensure_key_object(key)
        resource.delete_item(TableName=self.table_name, Key=key)

    def query(self, key_condition_expression, expression_attribute_values):
        response = resource.query(TableName=self.table_name, KeyConditionExpression=key_condition_expression, ExpressionAttributeValues=expression_attribute_values)
        return response.get('Items', [])

    def scan(self, filter_expression=None, expression_attribute_values=None):
        response = resource.scan(TableName=self.table_name, FilterExpression=filter_expression, ExpressionAttributeValues=expression_attribute_values)
        return response.get('Items', [])
    ...
    def _batch_get_itmes(self, keys):
        ...
        while len(list(query_data.keys())) > 0:
            response = resource.batch_get_item(RequestItems=query_data)
            query_data = response['UnprocessedKeys']
            items.extend(response.get('Responses', {}).get(self.table_name, []))
        items = [json_utils.ensure_dict(item) for item in items]
        return items
