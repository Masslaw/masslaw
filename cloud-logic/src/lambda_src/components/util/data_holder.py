...
class DataHolder:
    ...
    def get_data_property(self, key, default=None):
        if isinstance(key, str):
            key = [key]
        val = json_utils.get_from(self._get_data(), key, default)
        return val
    ...
    def get_data_properties(self, keys):
        return json_utils.select_keys(self._get_data(), keys)

    def delete_data_properties(self, keys):
        data = self._get_data()
        data = json_utils.delete_keys(data, keys)
        self._set_data(data)

    def delete_data_property(self, key):
        data = self._get_data()
        data = json_utils.delete_at(data, key)
        self._set_data(data)

    def keep_keys(self, keys):
        data = self._get_data()
        data = json_utils.select_keys(data, keys)
        self._set_data(data)
    ...
    def set_data_property(self, key: list, value):
        ...
        data = self._get_data()
        json_utils.set_at(data, key, value)
        self._set_data(data)

    def update_data(self, update_obj, create_new_keys=True):
        if not create_new_keys:
            existing_keys = list(self.get_data_copy().keys())
            update_obj = json_utils.select_keys(update_obj, existing_keys)
        update_obj = json_utils.delete_keys(update_obj, self._locked_attributes)
