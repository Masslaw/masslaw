...
class MasslawCaseManagementApiFileActionHandler(MasslawCaseManagementApiCaseActionHandler):
    ...
    def __assert_user_access_to_file(self):
        ...
        if self._file_id not in user_access_files: raise masslaw_case_users_management_exceptions.MasslawCaseUnauthorizedUserActionException(f"An attempt was made to access a file that the user does not have access to. File ID: {self._file_id}")
