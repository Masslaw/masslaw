...
neptune_client = NeptuneClient(read_connection=NeptuneConnection(connection_endpoint=neptune_read_endpoint, connection_port=8182, connection_protocol=neptune_connection_protocol, connection_type='gremlin'),
...
class MasslawCaseKnowledgeManager:
    ...
    def get_node_item_data(self, node_id: str) -> dict:
        ...
        with ThreadPoolExecutor() as executor:
            node_data_future = executor.submit(neptune_client.get_node_by_id, node_id)
            node_outgoing_connections_future = executor.submit(neptune_client.get_edges_by_nodes_connection, node_id, None)
            node_ingoing_connections_future = executor.submit(neptune_client.get_edges_by_nodes_connection, None, node_id)
        ...
        connected_nodes = []
        with ThreadPoolExecutor() as executor:
            for connection in node_outgoing_connections: to_nodes_futures.append(executor.submit(neptune_client.get_node_by_id, connection.get_to_node()))
            for connection in node_ingoing_connections: from_nodes_futures.append(executor.submit(neptune_client.get_node_by_id, connection.get_from_node()))
            for future in to_nodes_futures + from_nodes_futures: connected_nodes.append(future.result())
    ...
    def get_edge_item_data(self, edge_id: str):
        edge_data = neptune_client.get_edge_by_id(edge_id)
        with ThreadPoolExecutor() as executor:
            from_node_future = executor.submit(neptune_client.get_node_by_id, edge_data.get_from_node())
            to_node_future = executor.submit(neptune_client.get_node_by_id, edge_data.get_to_node())
