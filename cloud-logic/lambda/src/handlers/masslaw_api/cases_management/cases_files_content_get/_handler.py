...
class GetCaseFileContent(MasslawCaseManagementApiFileActionHandler):
    def __init__(self):
        MasslawCaseManagementApiFileActionHandler.__init__(self, default_response_body={'download_urls': {}}, request_query_string_parameters_structure={'content_paths': [str], })
        self.__content_paths = ''

    def _load_request_query_string_params(self):
        MasslawCaseManagementApiFileActionHandler._load_request_query_string_params(self)
    ...
    def _execute(self):
        ...
        with ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for content_path in self.__content_paths:
                submissions[content_path] = executor.submit(case_storage_manager.get_case_file_download_url, user_id, self._file_id, content_path)
