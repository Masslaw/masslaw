...
class StartCaseFileUpload(MasslawCaseManagementApiInvokedLambdaFunction):
    def __init__(self):
        MasslawCaseManagementApiInvokedLambdaFunction.__init__(self, default_response_body={'upload_id': '', 'upload_urls': '', 'file_id': '', }, request_body_structure={'case_id': [str], 'file_name': [str], 'path': [list, None], 'num_parts': [str, int], 'languages': [list], })
        self.__case_id = ''
        self.__file_name = ''
        self.__num_parts = 0
    ...
    def _execute(self):
        ...
        mp_upload_data = case_storage_manager.start_uploading_file(user_id=user_id, file_name=self.__file_name, num_parts=self.__num_parts, file_path=self.__path, file_languages=self.__languages)
