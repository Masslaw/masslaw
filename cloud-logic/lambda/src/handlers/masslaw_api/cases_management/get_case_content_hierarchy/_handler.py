...
class GetCaseContentHierarchy(MasslawCaseManagementApiInvokedLambdaFunction):
    def __init__(self):
        MasslawCaseManagementApiInvokedLambdaFunction.__init__(self, default_response_body={'hierarchy': {}}, request_query_string_parameters_structure={'case_id': [str]})
        self.__case_id = ''
    ...
    def _execute(self):
        ...
        case_content_hierarchy = case_data_collector.get_case_content_hierarchy()
        self._set_response_attribute([lambda_constants.EventKeys.BODY, 'hierarchy'], case_content_hierarchy)


def handler(event, context):
    handler_instance = GetCaseContentHierarchy()
