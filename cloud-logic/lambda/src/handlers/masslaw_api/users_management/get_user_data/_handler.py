...
class GetUserData(AuthenticatedMasslawUserHttpInvokedLambdaFunction):
    ...
    def __init__(self):
        AuthenticatedMasslawUserHttpInvokedLambdaFunction.__init__(
            self,
            default_response_body={'user_data': {}},
    ...
    def _load_request_path_params(self):
        ...
        self.__user_id = self._request_path_params.get('user_id', None)

    def _execute(self):
        AuthenticatedMasslawUserHttpInvokedLambdaFunction._execute(self)
        target_user_instance = self._caller_user_instance
        if self.__user_id is not None and self.__user_id != 'me': target_user_instance = MasslawUserInstance(user_id=self.__user_id)
