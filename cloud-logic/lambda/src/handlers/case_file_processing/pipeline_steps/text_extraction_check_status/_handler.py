...
class TextExtractionCheckStatus(MasslawStepFunctionCaseFilePipelineNodeHandler):

    def _execute(self):
        self.__file_instance = MasslawCaseFileInstance(self._file_id)
        text_extraction_job_ids = self.__file_instance.get_data_property(['processing', 'stage_metadata', 'text_extraction', 'job_ids'], [])
        describe_jobs_response = describe_jobs(text_extraction_job_ids)
        ...
        if 'FAILED' in job_statuses:
            ...
            self.__file_instance.set_data_property(['processing', 'stage_information', 'text_extraction', 'status'], 'succeeded')
        else:
