from src.handlers.case_file_processing.pipeline_steps.knowledge_extraction_start._submit_knowledge_extraction_job import submit_knowledge_extraction_job
...
class KnowledgeExtractionStart(MasslawStepFunctionCaseFilePipelineNodeHandler):
    ...
    def _execute(self):
        ...
        knowledge_extraction_job_ids = submit_knowledge_extraction_job(self.__file_instance, stage=self._stage)
        assert not any([job_id is None for job_id in knowledge_extraction_job_ids])
        self.__file_instance.set_data_property(['processing', 'stage_metadata', 'knowledge_extraction', 'job_ids'], knowledge_extraction_job_ids)
