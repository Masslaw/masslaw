...
def merge_records(merget_to: KnowledgeRecord, to_merge: KnowledgeRecord):
    for entity_to_merge in to_merge.get_entities():
        matching_entity = entity_matching.find_matching_entity_in_record(entity_to_merge, merget_to)
        if matching_entity is None:
            merget_to.add_entities([entity_to_merge])
            continue
    ...
    merget_to.add_connections(to_merge.get_connections())
