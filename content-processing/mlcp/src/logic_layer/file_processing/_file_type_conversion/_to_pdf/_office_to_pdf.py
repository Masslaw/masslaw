...
class OfficeToPdf(FileConverter):
    supported_file_types = {"doc", "docx", "ppt", "pptx", "xls", "xlsx"}
    output_file_type = "pdf"

    @classmethod
    @logger.process_function("Converting an office file to PDF")
    def _do_convert(cls, doc_path: str, output_file_path: str):
        ...
        if result.returncode != 0:
            raise Exception(f"Error in converting an office file to PDF: {result.stderr.decode('utf-8')}")
