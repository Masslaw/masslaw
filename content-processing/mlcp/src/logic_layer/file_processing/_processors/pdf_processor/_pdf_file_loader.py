...
class PdfFileLoader:
    ...
    def _load_page_image(self, page_num, width=1_440, dpi=400):
    ...
    def export_pages_as_images(self, output_directory: str, image_prefix='page_', export_format='png'):
        page_images = self.get_page_images()
    ...
    def export_pages_as_pdf_files(self, output_directory: str, file_prefix='page_'):
        with open(self._file, 'rb') as original_pdf_file:
            reader = pypdf.PdfReader(original_pdf_file)
            def export_page(page_num):
                writer = pypdf.PdfWriter()
                writer.add_page(reader.pages[page_num])
                output_filename = f"{file_prefix}{page_num}.pdf"
                output_path = file_system_utils.join_paths(output_directory, output_filename)
                with open(output_path, 'wb') as output_pdf_file:
                    writer.write(output_pdf_file)
            run_thread_batch(func=export_page, batch_inputs=list(range(self.get_num_pages())))

    @logger.process_function("Extracting existing PDF optical text structure")
    def extract_existing_optical_text_document(self):
    ...
    def get_optical_text_document(self) -> ExtractedOpticalTextDocument:
        return self._text_document

    @logger.process_function("Hiding exising elements in PDF page images")
    def create_page_images_with_no_text_elements(self) -> List[StorageCachedImage]:
        if not self._text_document:
            self.extract_existing_optical_text_document()
        hidden_images = run_thread_batch(func=self._create_page_image_with_no_text_elements, batch_inputs=list(range(self.get_num_pages())))
        return hidden_images

    def _create_page_image_with_no_text_elements(self, page_num: int) -> StorageCachedImage:
        original_page_image = self.get_page_images()[page_num]
        image_with_hidden_text_elements = original_page_image.duplicate()
        ...
        loaded_image = image_with_hidden_text_elements.get_image()
        ...
        image_with_hidden_text_elements.set_image(loaded_image)
        return image_with_hidden_text_elements
