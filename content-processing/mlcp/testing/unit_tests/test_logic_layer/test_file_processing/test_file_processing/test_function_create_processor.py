...
class TestFunctionCreateProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.image_processor_patcher = patch('logic_layer.file_processing._processors.image_processor.ImageProcessor')
        self.image_processor_class = self.image_processor_patcher.start()
    ...
    def test_create_processor_on_unknown_file(self):
        ...
        with self.assertRaises(NoProcessorForFileException):
    ...
    def test_create_processor_on_image_files(self):
        for file_type in ('.bmp', '.pbm', '.pgm', '.ppm', '.sr', '.ras', '.jpeg', '.jpg', '.jpe', '.jp2', '.tiff', '.tif', '.png', '.exr', '.hdr', '.pic', '.webp'):
            file = self._create_file_of_type(file_type)
            processor = create_processor(file, ['en'])
            self.image_processor_class.assert_called_once_with(file=file, languages=['en'])
            self.assertEqual(processor, self.image_processor_class.return_value)
            self.image_processor_class.reset_mock()
