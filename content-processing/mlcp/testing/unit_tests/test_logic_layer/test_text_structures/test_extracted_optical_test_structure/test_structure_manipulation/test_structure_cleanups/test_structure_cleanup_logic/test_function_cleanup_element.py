...
from logic_layer.text_structures.extracted_optical_text_structure.structure_manipulation._structure_cleanups._structure_cleanup_logic import cleanup_element


class TestFunctionCleanupElement(unittest.TestCase):
    def test_function_cleanup_element(self):
        ...
        cleanup_element(group1)

        self.assertEqual(len(group1.get_children()), 1)
        self.assertEqual(len(group1.get_children()[0].get_children()), 2)
